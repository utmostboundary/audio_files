...
from app.application.file_manager import FileManager
...
from app.infrastructure.databases.postgres.gateways.audio_file import (
    SqlAAudioFileGateway,
)
...
from app.infrastructure.file_manager.local_file_manager import LocalFileManager
...
def provide_config(provider: Provider) -> None:
    ...
    provider.from_context(LocalFileConfig, scope=Scope.APP)
...
def provide_infrastructure(provider: Provider) -> None:
    provider.provide(
        LocalFileManager,
        scope=Scope.REQUEST,
...
def setup_provider(provider: Provider) -> None:
    ...
    provide_interactors(provider=provider)
    provide_factories(provider=provider)
    provide_infrastructure(provider=provider)
